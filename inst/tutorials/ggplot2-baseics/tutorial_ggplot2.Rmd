...
description: > 
  **Japanese** Tutorial, データ分析勉強会の初学者向け講義資料『ggplot2のすゝめ』を対話型チュートリアル形式にしました。
---
...
　本チュートリアルは [データ分析勉強会]() での初学者向け講義で使用した『ggplot2 のすゝめ』を再編集したものです。  
　

## はじめに
　R にデフォルトで用意されているグラフ描画関数は便利ですが、色使いのセンスやパラメータの指定方法が統一性に欠けるなど初学者にとって分かりにくい点が多々あります。  
　例えば散布図を描くと下図のようになりシンプルですが味気ない面もあります。  

```{r}
plot(iris$Petal.Width, iris$Petal.Length)
```

　このような描画を改善し見やすくエレガントな描画を実現するのが `ggplot2` パッケージです。`ggplot2` は 'Create Elegant Data Visualisations Using the Grammar of Graphics' と謳っているように統一された文法も提供しています。  
　例えば上図と同様の散布図は以下のコードで描きます。品種（`Species`）による層別表示指定も簡単です。  
...
　では、実際にコードを実行しながら `ggplot2` パッケージを学んでいきましょう。なお、本チュートリアルでは `ggplot2` パッケージで定義されている関数であることを明示的に示すために関数表記を `ggplot2::function` という形式にしています。  



## 描画の基本
　`ggplot2` パッケージでは整然データ(tidy data)を統一した文法で扱うことが可能です。例えば前出の散布図を描くには  

1. 描画対象となるデータを指定
1. 描くグラフの座標軸になる変数を指定
1. 描くフラフを指定

　という手順で描きます。では、手順をなぞりながら実際にコードを実行してみましょう。  

　  

### 1. 描画対象となるデータを指定
　`iris` という、三種類のアヤメの花の花弁と萼片のを測定したデータを用います。[Run Code] をクリックして描画対象となるデータを確認してみましょう。  
...
### 2. 描くグラフの座標軸になる変数を指定
　次にグラフの横軸（$X$軸）に萼片（Sepal）の幅（Width）を縦軸（$Y$軸）に同じ萼片（Sepal）の長さ（Lenrth）を指定します。これでベースとなる座標が描けます。  
...
### 3. 描くフラフを指定
　最後に散布図を描きます。散布図を描くには `ggplot2::geom_point` という関数を指定します。  
...
　このように `ggplot2` パッケージはデジタル地図で使われるレイヤー構造のように指定することで様々なグラフを描けます。では、次に箱ひげ図を描いてみましょう。  
