---
title: "R Basics (Japanese Tutorial)"
output:
  learnr::tutorial:
    df_print: "paged"
    highlight: textmate
...
description: > 
  データ分析勉強会で使用した『RStudioのすゝめ』を再編成しチュートリアル化したものです。
---
...
## はじめに
　本ファイルは[データ分析勉強会][0]で使用した『RStudioのすゝめ』の講義資料を再整理し`learnr`パッケージを用いて対話型チュートリアル形式に編集しなおした初学者向けの資料です。実際にRのコードを入力して実行することが可能です。  


### 実行環境
　RStudio v1.3以降のみサポートしています。`kmetrics`パッケージがインストールされていれば、RStudioの「Tutorial」タブに実行可能なチュートリアルが表示されます。タイトルの右側に表示される［Start Tutorial］ボタンをクリックしてください。
